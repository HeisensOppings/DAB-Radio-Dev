...
public:
    ...
    virtual void OnEnsemble_1_ID(
        ...
        const uint8_t occurance_change) = 0;
    ...
    virtual void OnServiceComponent_3_Short_Language(
        const subchannel_id_t subchannel_id, const uint8_t language) = 0;
    ...
    virtual void OnServiceLinkage_1_LSN_Only(
        ...
        const lsn_t linkage_set_number) = 0;
    ...
    virtual void OnServiceLinkage_1_RDS_PI_ID(
        const bool is_active_link, const bool is_hard_link, const bool is_international,
        const lsn_t linkage_set_number,
        const fm_id_t rds_pi_id, uint8_t extended_country_code=0x00) = 0;
    virtual void OnServiceLinkage_1_DRM_ID(
        const bool is_active_link, const bool is_hard_link, const bool is_international,
        const lsn_t linkage_set_number,
        const drm_id_t drm_id) = 0;
    ...
    virtual void OnServiceComponent_4_Short_Definition(
        ...
        const subchannel_id_t subchannel_id) = 0;
    // For packet mode service components that have a global id
    virtual void OnServiceComponent_4_Long_Definition(
        const ServiceId service_id,
        const service_component_id_t service_component_id,
        const service_component_global_id_t service_component_global_id) = 0;
    ...
    virtual void OnSubchannel_2_FEC(
        const subchannel_id_t subchannel_id, const uint8_t fec_type) = 0;
    ...
    virtual void OnAnnouncementSwitching_1(
    ...
    virtual void OnOtherEnsemble_1_Service(
        const ServiceId service_id,
        const EnsembleId ensemble_id) = 0;
