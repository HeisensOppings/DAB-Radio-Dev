...
public:
    ...
    void OnServiceLinkage_1_LSN_Only(
        ...
        const lsn_t linkage_set_number) override;
    void OnServiceLinkage_1_ServiceID(
        const bool is_active_link, const bool is_hard_link, const bool is_international,
        const lsn_t linkage_set_number,
        const ServiceId service_id) override;
    void OnServiceLinkage_1_RDS_PI_ID(
        const bool is_active_link, const bool is_hard_link, const bool is_international,
        const lsn_t linkage_set_number,
        const fm_id_t rds_pi_id, uint8_t extended_country_code=0x00) override;
    void OnServiceLinkage_1_DRM_ID(
        const bool is_active_link, const bool is_hard_link, const bool is_international,
        const lsn_t linkage_set_number,
        const drm_id_t drm_id) override;
    ...
    void OnServiceComponent_4_Long_Definition(
        const ServiceId service_id,
        const service_component_id_t service_component_id,
        const service_component_global_id_t service_component_global_id) override;
    ...
    void OnEnsemble_4_ExtendedLabel(
        const EnsembleId ensemble_id,
    ...
    void OnService_4_ExtendedLabel(
        const ServiceId service_id,
        const uint8_t toggle_flag, const uint8_t segment_index, const uint8_t rfu,
        const uint8_t* buf, const uint8_t N) override;
    // fig 2/4 - Non-primary service component label
    void OnServiceComponent_7_ExtendedLabel(
        const ServiceId service_id,
        const service_component_id_t service_component_id,
         const uint8_t toggle_flag, const uint8_t segment_index, const uint8_t rfu,
        const uint8_t* buf, const uint8_t N) override;
